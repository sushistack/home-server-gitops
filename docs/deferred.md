...
- 어느 작업인지 미확정. 후보(확인 명령은 검증 절):
  ...
  3. **replica rebuild** — ⚠️ `ae83cff`의 `dataLocality: best-effort`가 레플리카 재배치 재빌드를
     트리거했을 수 있음(일회성, 끝나면 멈춤).

### H6 (상시 바닥) — `netdata` 상시 CPU ~25%

- 근거: 두 스냅샷 모두 ~25%로 **평평** → 스파이크 아닌 상시 부하. 팬이 "계속" 도는 연속 성분의 주범.
- 모니터링 에이전트치고 과함. 추정 원인: **ML 이상탐지 + ebpf 플러그인 + per-second 수집**이
  99개 컨테이너에 곱해짐. (확인: `kubectl -n netdata get cm`로 설정 형태 파악)
...
### S4 — netdata 튜닝  *(H6 대응 — ROI 최고, 손해 거의 0)*

상시 ~25% 회수가 목표. 기대: **25% → 한 자릿수.** 셋 다 홈랩에선 없어도 되는 기본값.

| 설정 (netdata.conf) | 효과 | 잃는 것 |
|---|---|---|
| `[ml] enabled = no` | **큼** (신버전 기본 ON) | 자동 이상탐지 |
| `[plugins] ebpf = no` | **큼** | 커널레벨 syscall/네트워크 상세 |
| `[global] update every = 5` (기본 1) | 수집 CPU ≈1/5 | 1초→5초 해상도 |

- 전제: netdata가 ArgoCD 관리이고 설정이 ConfigMap인지 먼저 확인(`kubectl -n netdata get cm`).
- 팬의 **연속 성분**을 직접 깎는 작업이라 체감 가장 클 것 → **1순위로 적용 권장.**
...
| **dataLocality 재빌드** | **튜닝 불필요 — 일회성, 끝나면 멈춤** | 없음(기다리면 됨) |
| 백업 빈도 | 12h → 24h | RPO 늘어남 |

- ⚠️ 만약 dataLocality 재빌드면 며칠 지켜보면 자연 소멸 → 성급히 끄지 말 것.
...
- **다음 액션:** (1) S4 netdata 튜닝 — 설정 ConfigMap 확인 후 PR. (2) 검증 #6으로 H5 longhorn 작업
  확정 → S5 선택 적용. 단 dataLocality 재빌드면 자연 소멸이라 대기.
