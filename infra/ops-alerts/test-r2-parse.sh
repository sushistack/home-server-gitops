...
# (ntfy newest 1h ago; navidrome newest 20h ago; miniflux newest 2h ago but oldest 40d ago)
FIXTURE='[
...
OUT=$(printf '%s' "$FIXTURE" | jq -r --argjson now "$NOW" --argjson stale "$STALE" --argjson ret "$RET" '
